...
class AtCmdCheckAttach(AtCmd):
    def __init__(self, connection=None, operation='execute'):
        """Create instance of AtCmdDetach class"""
        super(AtCmdCheckAttach, self).__init__(connection, operation)
        if operation != 'execute':
            raise AtCommandModeNotSupported("{} operation no supported for: {}".format(operation, self))
        self.set_at_command_string(command_base_string="AT+CGATT?")
        #TODO: fix timeout for check attach
        self.timeout = 180
